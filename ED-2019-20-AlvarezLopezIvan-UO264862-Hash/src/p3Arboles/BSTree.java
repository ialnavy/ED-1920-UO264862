...
public class BSTree<T extends Comparable<T>> {
	...
	/**
	 * Un arbol es perfectamente equilibrado (APE) si para TO DO nodo del arbol, la
	 * diferencia (en valor absoluto) entre el numero de nodos de cada una de sus
	 * ramas es menor o igual que 1
	 * 
	 * @return true si el arbol es APE y false en caso contrario
	 */
	public boolean isAPE() {
		return isAPE(this.root);
	}

	/**
	 * Evalua si es APE un subarbol dada su subraiz
	 * 
	 * @param subRoot Subraiz del arbol
	 * @return Es APE?
	 */
	private boolean isAPE(BSTNode<T> subRoot) {
		if (subRoot == null)
			return true;
		int leftSons = numNodes(subRoot.getLeft());
		int rightSons = numNodes(subRoot.getRight());
		return (Math.abs(leftSons - rightSons) <= 1) && isAPE(subRoot.getLeft()) && isAPE(subRoot.getRight());
	}

	/**
	 * Devuelve la cantidad de nodos que hay en un subarbol dada su subraiz
	 * 
	 * @param subRoot Subraiz del arbol
	 * @return Cantidad de nodos del subarbol
	 */
	private int numNodes(BSTNode<T> subRoot) {
		if (subRoot == null)
			return 0;
		return (1 + numNodes(subRoot.getLeft()) + numNodes(subRoot.getRight()));
	}

	/**
	 * Devuelve la altura del arbol en un momento dado
	 * 
	...
	/**
	 * Metodo auxiliar que calcula recursivamente la altura de un subarbol
	 * 
	 * @param subRoot Raiz del subarbol
	 * @return Altura del subarbol
	 */
	private int getHeightRecursive(BSTNode<T> subRoot) {
		if (subRoot == null)
			return 0;
		return Math.max(getHeightRecursive(subRoot.getLeft()), getHeightRecursive(subRoot.getRight())) + 1;
	}
